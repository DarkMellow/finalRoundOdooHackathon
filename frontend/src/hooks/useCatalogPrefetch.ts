...
function mapApiProductToProduct(item: ApiProduct): Product {
  ...
  return {
    ...
    brand: brandName,
    image: coverImg,
    price: Math.round(item.discounted_price || price),
    originalPrice: item.discount_percent && item.discount_percent > 0 ? Math.round(item.sales_price || price) : undefined,
    discount: item.discount_percent ? Math.round(item.discount_percent) : undefined,
