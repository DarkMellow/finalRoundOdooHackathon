...
// ---------------------------------------------------------------------------
// Mock data — replace via fetchVendorProfile() when backend is ready.
// ---------------------------------------------------------------------------
...
export async function fetchVendorProfile(): Promise<VendorProfileData> {
  ...
  return new Promise((resolve) => {
    setTimeout(() => resolve({ ...mockVendorProfile }), 350)
  })
}
...
export async function uploadVendorAsset(
  ...
  return new Promise((resolve) => {
    setTimeout(() => resolve({ url: mockVendorProfile.profileImageUrl ?? "" }), 500)
  })
}
